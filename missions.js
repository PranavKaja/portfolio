...
(function () {
  ...
  function wireOverlay(byCode, grid) {
    ...
    function open(num, isKeyboard) {
      ...
      if (ghBox) {
        if (m.github_url) {
          ghBox.innerHTML = `<a href="${esc(m.github_url)}" target="_blank" rel="noopener noreferrer" class="github-btn" style="display: inline-flex; align-items: center; gap: 8px; background: transparent; border: 2px solid var(--text-main); padding: 10px 16px; font-family: 'Share Tech Mono', monospace; font-weight: bold; font-size: 0.85rem; color: var(--text-main); text-transform: uppercase; transition: all 0.2s;">
